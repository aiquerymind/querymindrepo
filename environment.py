...
# from .LLM import complete_text_claude  # Removed for Gemini-only setup
# from .prepare_task import prepare_task, get_task_info  # Removed for deployment
...
"""
    # response = complete_text_claude(prompt, stop_sequences=["[end]"], log_file=log_file)  # Removed for Gemini-only setup
...
class Environment:
    def __init__(self, args):
        ...
        if not args.interactive:
            # Set research_problem and benchmark_folder_name directly
            self._research_problem = args.problem
            # Generate a simple folder name from the problem (first 8 alphanumeric chars)
            import re
            safe_name = re.sub(r'\W+', '_', self._research_problem)[:32]
            self._benchmark_folder_name = safe_name or "default_benchmark"
    ...
    def _initialize_task_env(self):
        ...
        # No prepare_task or benchmark_dir logic needed for deployment
        # Just create the work_dir
    ...
    def __enter__(self):
        # Timeout not supported on Windows, so skip SIGALRM
        return self
